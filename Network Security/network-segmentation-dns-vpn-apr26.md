...
An earlier project implemented network-wide DNS filtering via NextDNS over DNS-over-TLS (see [NextDNS with DoT Implementation](nextdns-dot-setup-apr26.md)). That work established DNS filtering and basic encrypted DNS, but had limitations: a single flat network with all devices on one subnet, no per-device visibility in NextDNS logs, and no VPN routing capability.

This project addresses those limitations by replacing stock router firmware with a more flexible firmware option, introducing multiple network segments with different trust levels, and moving from DoT to the NextDNS CLI for per-device DNS visibility.
